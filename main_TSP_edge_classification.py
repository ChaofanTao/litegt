...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    from train.train_TSP_edge_classification import train_epoch, evaluate_network

    train_loader = DataLoader(trainset, batch_size=params['batch_size'], shuffle=False, collate_fn=dataset.collate, num_workers=0)
    ...
    if params['resume']:
        ...
        epoch_val_f1s = checkpoint['epoch_val_accs']
        epoch_test_f1s = checkpoint['epoch_test_accs']
        epoch_train_f1s = checkpoint['epoch_train_accs']
    ...
    try:
        with tqdm(range(start_epoch+1, params['epochs'])) as t:
            for epoch in t:
                ...
                logging.info("""Epoch: {}, lr: {}, train/_loss: {}, val/_loss: {}, train/_f1: {}, val/_f1: {},
                 test/_f1: {}"""\
                 .format(epoch, optimizer.param_groups[0]['lr'], epoch_train_loss, epoch_val_loss, 
                            epoch_train_f1, epoch_val_f1, epoch_test_f1))
                ...
                checkpoint = {
                        ...
                        "epoch": epoch,
                        "scheduler": scheduler,
                        "epoch_val_accs": epoch_val_f1s,
                        "epoch_test_accs": epoch_test_f1s,
                        "epoch_train_accs": epoch_train_f1s,
    ...
    logging.info("Test f1: {:.4f}".format(test_f1))
    logging.info("Train f1: {:.4f}".format(train_f1))
    ...
    best_val_id = epoch_val_f1s.index(max(epoch_val_f1s))
    logging.info("Best Test f1: {:.4f}".format(epoch_test_f1s[best_val_id]))
    logging.info("Best Train f1: {:.4f}".format(epoch_train_f1s[best_val_id]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="TSP", help="Please give a value for dataset name")
    parser.add_argument('--out_dir', type=str, default="out/TSP/", help="Please give a value for out_dir")
    ...
    parser.add_argument('--init_lr', type=float, default=0.001, help="Please give a value for init_lr")
    parser.add_argument('--lr_reduce_factor', type=float, default=0.5, help="Please give a value for lr_reduce_factor")
    parser.add_argument('--lr_schedule_patience', type=int, default=15, help="Please give a value for lr_schedule_patience")
    ...
    parser.add_argument('--pos_enc_dim', type=int, default=8, help="Please give a value for pos_enc_dim")
